...
tmux send-keys -t $SESSION:simulator "roslaunch sunray_simulator sunray_sim_uav_planning.launch rviz_enable:=false" C-m
...
# 窗口6：通信桥接
tmux new-window -t $SESSION -n "ego-planner"
tmux send-keys -t $SESSION:ego-planner "roslaunch sunray_simulator sunray_sim_ego.launch" C-m
